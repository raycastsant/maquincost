	...
	public function renderStatement($type, $data) {
		...
		switch (strtolower($type)) {
			...
			case 'schema':
				...
				return "CREATE TABLE {$table} (\n{$columns}{$indexes}) {$tableParameters} ENGINE=InnoDB  DEFAULT CHARSET=latin1;";
			case 'alter':
				return;
		}
	}
 ...
 * @return string
 */
	public function createSchema($schema, $tableName = null,$delimiterSql = "") {
		...
		foreach ($schema->tables as $curTable => $columns) {
			if (!$tableName || $tableName == $curTable) {
				...
				$columns = $cols;
				$out .= $this->renderStatement('schema', compact('table', 'columns', 'indexes', 'tableParameters')) .$delimiterSql."\n\n";
			}
		}
		return $out;
	}
