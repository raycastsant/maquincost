...
<?php echo $this->Form->create('Pieza', array('class'=>'form-horizontal')); ?>
	<fieldset>
		<div class="head-form "><h4></a><?php echo __('Editar Pieza'); ?></h4></div>
        <br />
        
    <div class="span2"><a href="#modaldialog" role="button" data-toggle="modal"><img id="piezaicon" src="/maquincost/img/parts/<?php echo $this->request->data['Pieza']['imagen']; ?>" style="width: 80px; height: 80px;" class="img-rounded img-polaroid" data-toggle="tooltip" title="Click para seleccionar imagen"/></a>
            <input id="PiezaImagen" value="<?php echo $this->request->data['Pieza']['imagen']; ?>" type="hidden" name="data[Pieza][imagen]"/> 
    </div>
    <br />
	<?php
            ...
            ?>  
                 <table cellpadding='0' cellspacing='0' class='table table-bordered table-condensed'>
                  <?php 
                    ...
                    {
                        $tdcode = '<td><a id="'.substr($filelist[$i], 0, strlen($filelist[$i])-4).'"><img src="/maquincost/img/parts/'.$filelist[$i].'" style="width: 80px; height: 80px; cursor: pointer;" class="img-rounded img-polaroid"/></a></td>';
    ...
    <script>
       ...
       <?php
        ...
        ?>
           ...
           {
             $("#piezaicon").attr('src', '/maquincost/img/parts/<?php echo $filelist[$i]?>');
